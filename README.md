...
2008~2009년 서비스됐던 전략 바둑 대전 게임 **바투**의 웹 리메이크입니다.
바둑 룰을 기반으로 **±화점 · 베이스빌드 · 턴베팅 · 히든 · 스캔**이 더해진 게임입니다.
...
| ±화점 | +에 돌을 두면 +5점, −는 −5점. **처음 놓을 때 1회만** 적용되고 화점은 사라짐(공식 규칙). 남은 −점은 집으로 계산 안 됨 |
...
| 초읽기 | (옵션) 매 수 25초×3회(WBL 공식 규격) 또는 15초×3회. 1회 소진마다 −2점, 전부 소진하면 시간패 |
...
├─ css/style.css   # 스타일
├─ js/
│  ├─ maps.js      # 맵 정의 (제네시스/클래식/세렝게티)
│  ├─ engine.js    # 룰 엔진 (바둑 룰 + 바투 특수 룰, DOM 비의존)
│  ├─ ui.js        # 캔버스 보드 렌더러
